...
--------------------------------------------------------
  ...
  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "ORDSDEMO"."CALLER_CRUDWSDEPT" 
IS 
  ...
  IS     
   ...
   BEGIN 
    ...
    ordsdemo.devora_logger_pack$.set_logger('apex'); 
    apex_debug.enable(p_level => apex_debug.c_log_level_info);
    ...
    IF OAUTH_DEPT_TOKEN IS  NULL THEN  
        ...
        */
         ...
         -- Version PL/sql de Action: Execute Server - side Code.  
          ...
          "Items to Return" para que funcione la instrucción  APEX_UTIL.SET_SESSION_STATE. 
    ...
    END IF;
     ...
     When Others Then
       ...
       /*
        ...
        p_message => 'Call_GETDepts_API => provocado division por cero cachado y añadido mensaje usando apex_error.add_error, 
   ...
   BEGIN 
    ...
    END IF;
     ...
     When Others Then
       ...
       /*
        apex_error.add_error (
        p_message => 'Call_PUTPOSTDepts_API => provocado division por cero cachado y añadido mensaje usando apex_error.add_error, 
           pero en front se muestra como alert normal. Desde CS se llama como apex.event.trigger ', 
        p_display_location => apex_error.c_on_error_page);
       */
