...
/*
Vka123  nip vb win10. al 25 02 2021. 
begin 
 devora_logger_pack$.set_logger($$plsql_line, $$plsql_unit , 'plsql'); 
 raise no_data_found; -- force throws exception 
exception		 
    when no_data_found then 
	 devora_logger_pack$.log_msg ('p_dbg_msg'); 
end;

GRANT EXECUTE ON ORDSDEMO.devora_logger_pack$ TO  devasch;
ordsdemo.devora_logger_pack$.set_logger($$plsql_line, $$plsql_unit , 'apex'); 

ordsdemo.devora_logger_pack$.log_msg ('p_dbg_msg:'  || V('P2_TEXT')  || v_msg) ; 

--Select * from devora_logger$

	   select *  from APEX_200200.APEX_APPLICATION_PAGES 
	     where page_id = 15 and application_id = 101;


select * from apex_200200.apex_workspaces;
